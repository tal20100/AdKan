    ...
    private func createGroup() {
        ...
        Task {
            do {
                ...
                onCreated?(newGroup)
                dismiss()
            } catch let error as GroupServiceError where error.isForeignKeyViolation {
                // User row missing — ensure it exists, then retry once
                do {
                    try await services.auth.ensureUserRow()
                    let newGroup = try await services.groups.createGroup(name: groupName, type: groupType)
                    UIImpactFeedbackGenerator(style: .medium).impactOccurred()
                    onCreated?(newGroup)
                    dismiss()
